...
const getBestSellers = async (req, res) => {
    try {
        ...
        const tag = await Tag.findOne({ name: /best seller|bestseller/i });
        ...
        const products = await Product.find(queryFilter)
            ...
            .populate('brand', 'name slug')
            .populate('tags', 'name')
            .select('title price salePrice images slug stock averageRating freeShipping isDod dodPrice dodStart dodEnd createdAt')
...
const getFeaturedProducts = async (req, res) => {
    try {
        ...
        const filter = featuredTag ? { tags: { $in: [featuredTag._id] } } : {};
...
const getNewArrivals = async (req, res) => {
    try {
        ...
        const filter = newTag ? { tags: { $in: [newTag._id] } } : {};
