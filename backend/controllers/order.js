...
const creatOrder = async (req, res) => {
  try {
    ...
    // Clean and validate mobile number (supports +923337494323, 923337494323, 03337494323, etc.)
    const rawMobile = shippingAddress.mobile || '';
    const cleanMobile = String(rawMobile).trim().replace(/[\s\-\(\)]/g, '');
    const mobileRegex = /^(\+92|92|0)?3\d{9}$/;

    if (!cleanMobile || !mobileRegex.test(cleanMobile)) {
      return res.status(400).json({
        success: false,
        message: "Please enter a valid Pakistani mobile number (e.g., 03337494323 or +923337494323).",
        error: "Invalid Pakistani mobile number."
      });
    }

    // Determine guest vs logged-in user safely
    let userId = req.body.userId;
    const isValidUser = userId && mongoose.Types.ObjectId.isValid(userId) && userId !== "guest";
    const isGuest = !isValidUser;
    let guestId = null;

    if (isGuest) {
      guestId = req.cookies?.guestId || req.get("X-Guest-Id") || req.body.guestId || uuidv4();
    ...
    // Validate stock and product existence before proceeding
    for (const item of cartSummary) {
      const pId = item.productId || item.product || item._id;
      if (!pId || !mongoose.Types.ObjectId.isValid(pId)) {
        return res.status(400).json({
          success: false,
          message: `Invalid product in cart: ${item.title || 'Unknown product'}`,
          error: `Invalid product ID: ${pId}`
        });
      }

      console.log("Checking stock for product:", pId);
      const product = await Product.findById(pId);
      if (!product) {
        return res.status(404).json({
          success: false,
          message: `Product "${item.title || 'Item'}" is no longer available.`,
          error: `Product not found: ${pId}`
        });
      }
      if (product.stock < Number(item.count || 1)) {
        return res.status(400).json({
          success: false,
    ...
    const bulkOperations = cartSummary.map((item) => {
      ...
      return {
        updateOne: {
          ...
          update: {
            $inc: {
              stock: -Number(item.count || 1),
              sold: Number(item.count || 1),
            },
          },
        },
      };
    ...
    try {
      ...
      orderShortId = (seqDoc?.seq || 0) + 1000;
    } catch (counterError) {
      console.warn("Counter sequence error, using fallback timestamp ID:", counterError?.message);
      orderShortId = Math.floor(1000 + (Date.now() % 900000));
    }

    // Format cartSummary to ensure 'product' ObjectId and all required fields are set
    const formattedCartSummary = cartSummary.map((item) => ({
      product: item.productId || item.product || item._id,
      title: item.title || "Product",
      image: typeof item.image === "object" ? item.image?.url || "" : item.image || "",
      count: Number(item.count || 1),
      price: Number(item.price || 0),
      selectedVariants: Array.isArray(item.selectedVariants) ? item.selectedVariants : [],
    }));
    ...
    const formattedShippingAddress = {
      ...
      mobile: cleanMobile,
      additionalInstructions: shippingAddress.additionalInstructions ? String(shippingAddress.additionalInstructions).trim() : undefined,
    };
...
const getRecentOrders = async (req, res) => {
  try {
    ...
    const orders = await Order.find({ orderedAt: { $gte: last24Hours } })
      .populate("cartSummary.product", "title price")
      .populate("orderedBy", "username mobile streetAddress")
      .sort({ orderedAt: -1 })
      .lean();
